...
const UserList = () => {
    ...
    if (isLoading) {
        return (
            <div className="flex justify-center items-center h-64">
                ...
                <p className="text-blue-700 text-lg font-semibold">Loading users...</p>
            </div>
        );
    }
    ...
    return (
        <div className="overflow-x-auto px-4 sm:px-6 pb-10">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-700">User Management</h2>
                {hasPermission('user:create') && (
                    <button onClick={() => navigate('/users/add')} className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg shadow transition duration-300">
                        <IconPlusCircle />
                        Add User
                    </button>
                )}
                    ...
                    <tbody>
                        {users?.map((user: any, index: number) => (
                            <tr key={user.id} className="border-b border-gray-100 hover:bg-blue-50 transition duration-150">
                                ...
                                <td className="py-4 px-6 text-center font-medium text-gray-700">{user.name}</td>
                                <td className="py-4 px-6 text-center text-gray-600">{user.email}</td>
                                        ...
                                        <div className="flex justify-center gap-3">
                                            ...
                                            {hasPermission('delete user') && (
                                                <button
                                                    ...
                                                    } text-white px-3 py-1 rounded-md flex items-center gap-1 transition`}
